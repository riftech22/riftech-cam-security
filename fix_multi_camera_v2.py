#!/usr/bin/env python3
"""Test script untuk analisa multi-kamera split frame - FFmpeg CLI version."""
...
def capture_frame_with_ffmpeg(rtsp_url, output_path="frame_temp.jpg", timeout=10):
    ...
    print(f"Capturing frame with FFmpeg CLI...")
    ...
    try:
        ...
        if os.path.exists(output_path):
            ...
            if frame is not None:
                ...
                print("❌ Failed to read frame from file")
                return None
        else:
...
def analyze_multi_camera_split():
    ...
    # Capture frame dengan FFmpeg
    frame = capture_frame_with_ffmpeg(rtsp_url)
    
    if frame is None:
        print("\n❌ Failed to capture frame from RTSP")
        print("\n💡 Alternative: Try capturing with existing service frames")
        
        # Cek ada frame dari running service?
        try:
        ...
        print("✅ Successfully captured frame from RTSP with FFmpeg")
    ...
    try:
        ...
        if persons_top > 0 and persons_bottom > 0:
            ...
            print(f"   ⚠️  Only TOP camera detected")
        elif persons_bottom > 0:
            print(f"   ⚠️  Only BOTTOM camera detected")
        else:
...
if __name__ == "__main__":
    print("=" * 60)
    print("Multi-Camera Split Analysis Tool")
    print("=" * 60)
    print()
